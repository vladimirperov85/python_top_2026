...
from sqlalchemy.orm import DeclarativeBase, Session, Mapped, mapped_column, relationship,selectinload
...
class StoreManager:
    ...
    def add_manufacturer(self, name: str) -> Manufacturer:  # Исправлено: было get_manufacturer с дублированием
        ...
        saved_manufacturer = self._save_obj(manufacturer)  # Исправлено: было save_obj, нужно _save_obj
        logger.info(f'Производитель {saved_manufacturer.name} добавлен')
        return saved_manufacturer
        
    def get_manufacturers(self, stmt=None) -> List[Manufacturer]:  # Исправлено: было дублирование имени метода
        if stmt is None:
    ...
    def delete_manufacturer(self, manufacturer_id: int) -> bool:
        ...
        if manufacturer.products:
            logger.warning(f'Производитель {manufacturer.id} имеет товары, удаление невозможно, так как есть {len(manufacturer.products)} товаров')
            return False
    ...
    def add_product(self, name: str,manufacturer_id:int,category:str,price:Decimal|float|int,serial_number:Optional[str] = None) -> Product:
        ...
        try:
            ...
            if 'unique constrain failed' in str(e).lower():
    ...
    def find_product_by_id(self, product_id: int) -> Optional[Product]:
        if not isinstance(product_id, int) or product_id <= 0:
            raise ValueError(f'{product_id} должно быть целым числом')
        
        stmt = select(Product).where(Product_id=product_id).options(
            selectinload(Product.manufacturer,Product.orders)
        )
        return self._fetch_one(stmt)
    ...
    def find_products_by_name_or_category(self,query:str)-> List[Product]:
        ...
        stmt = select(Product).where(
            Product.name.ilike(f'%{query.strip()}%') | Product.category.ilike(f'%{query.strip()}%')
        )
        return self._fetch_all(stmt)
    ...
    def update_product(self,
        ...
        new_serial_number: Optional[str] = None) -> bool:
        ...
        try:
            ...
            if 'unique constraint failed' in str(e).lower():
                raise ValueError(f'Товар с таким серийным номером {new_serial_number} уже существует')
